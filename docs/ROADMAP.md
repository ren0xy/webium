...
## v0.2.0 - Richer Rendering & CSS

Expand rendering fidelity and CSS support beyond the hello world baseline.

Planned direction:
- Richer CSS property support (borders, margins, overflow, opacity, visibility, display modes)
- Pseudo-classes (`:hover`, `:active`, `:focus`) and pseudo-elements
- More HTML elements (form inputs, images with asset loading, tables)
- Flexbox completeness (flex-wrap, align-content, gap, order)
- Image/asset loading pipeline (sprites, textures from UI project folders)
- Scroll containers and overflow handling
- Improved text rendering (inline spans, line-height, text-decoration, text-overflow)
