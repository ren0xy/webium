...
namespace Webium.Unity
{
    ...
    /// </summary>
    public class UGUIRenderCommandExecutor : IRenderCommandExecutor
    {
        ...
        private readonly Dictionary<int, Dictionary<string, string>> _appliedStyles = new();
        ...
        public IReadOnlyDictionary<int, Dictionary<string, string>> AppliedStyles => _appliedStyles;

        public UGUIRenderCommandExecutor(Transform rootTransform)
        {
        ...
        private void ExecuteDestroy(RenderCommand cmd)
        {
            if (_nodeObjects.TryGetValue(cmd.NodeId, out var go))
            {
                ...
                _appliedStyles.Remove(cmd.NodeId);
            }
        }
        ...
        private void ExecuteUpdateStyle(RenderCommand cmd)
        {
            ...
            UGUIStyleApplier.Apply(go, cmd.Styles);
            ...
            ApplyBorder(go, cmd.Styles);

            // Cache all applied styles per node for later use (e.g., when creating __WebiumText children)
            if (!_appliedStyles.TryGetValue(cmd.NodeId, out var cached))
            {
                cached = new Dictionary<string, string>();
                _appliedStyles[cmd.NodeId] = cached;
            }
            foreach (var kvp in cmd.Styles)
            {
        ...
        private void ExecuteUpdateText(RenderCommand cmd)
        {
            ...
            else
            {
                ...
                if (childText == null)
                {
                    ...
                    // Apply cached text-related styles from the parent node
                    if (_appliedStyles.TryGetValue(cmd.NodeId, out var cachedStyles))
                    {
                        if (cachedStyles.TryGetValue("color", out var cachedColor))
                            tmp.color = CSSColorParser.Parse(cachedColor);
                        if (cachedStyles.TryGetValue("font-size", out var cachedFontSize))
                        {
                            var px = CSSUnitParser.ParsePx(cachedFontSize);
                            if (px > 0f) tmp.fontSize = px;
                        }
                        if (cachedStyles.TryGetValue("font-family", out var cachedFontFamily))
                        {
                            var font = CSSFontFamilyResolver.ResolveTMP(cachedFontFamily);
        ...
        private static void ApplyBackgroundColor(GameObject go, string value)
        {
            ...
            if (image == null)
            {
                // Cannot add Image if a Graphic subclass (e.g. TextMeshProUGUI) already exists
        ...
        private static void ApplyFontFamily(GameObject go, string value)
        {
            var tmp = go.GetComponent<TextMeshProUGUI>();
            if (tmp == null)
            {
            ...
            if (tmp != null)
            {
                var font = CSSFontFamilyResolver.ResolveTMP(value);
                if (font != null)
                    tmp.font = font;
            }
        }
        ...
        private static void ApplyBorder(GameObject go, Dictionary<string, string> styles)
        {
            float top = styles.TryGetValue("border-top-width", out var btw) ? CSSUnitParser.ParsePx(btw) : 0f;
            float right = styles.TryGetValue("border-right-width", out var brw) ? CSSUnitParser.ParsePx(brw) : 0f;
            float bottom = styles.TryGetValue("border-bottom-width", out var bbw) ? CSSUnitParser.ParsePx(bbw) : 0f;
            float left = styles.TryGetValue("border-left-width", out var blw) ? CSSUnitParser.ParsePx(blw) : 0f;

            if (top <= 0 && right <= 0 && bottom <= 0 && left <= 0) return;

            // Resolve border color: explicit border-color, or fall back to currentColor (text color)
            Color borderColor;
            if (styles.TryGetValue("border-color", out var bc))
            {
                borderColor = CSSColorParser.Parse(bc);
            }
            else
            {
                // CSS default: border-color is currentColor
                var tmp = go.GetComponentInChildren<TextMeshProUGUI>();
                borderColor = tmp != null ? tmp.color : Color.white;
            }

            if (top > 0) CreateBorderEdge(go.transform, "__border_top", borderColor,
                new Vector2(0, 1), new Vector2(1, 1), new Vector2(0, 1), Vector2.zero, new Vector2(0, top));
            if (bottom > 0) CreateBorderEdge(go.transform, "__border_bottom", borderColor,
                new Vector2(0, 0), new Vector2(1, 0), new Vector2(0, 0), Vector2.zero, new Vector2(0, bottom));
            if (left > 0) CreateBorderEdge(go.transform, "__border_left", borderColor,
                new Vector2(0, 0), new Vector2(0, 1), new Vector2(0, 0.5f), Vector2.zero, new Vector2(left, 0));
            if (right > 0) CreateBorderEdge(go.transform, "__border_right", borderColor,
                new Vector2(1, 0), new Vector2(1, 1), new Vector2(1, 0.5f), Vector2.zero, new Vector2(right, 0));
        }
        ...
        {
            // Reuse existing edge if style is re-applied
            var existing = parent.Find(name);
            GameObject edgeGo;
            if (existing != null)
            {
                edgeGo = existing.gameObject;
            }
            else
            {
                edgeGo = new GameObject(name);
                edgeGo.AddComponent<RectTransform>();
