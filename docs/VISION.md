...
- Parse HTML/CSS/JS from files and render them as native engine visuals at runtime.
- Provide a browser-compatible API surface (`document`, `element`, `event`) so standard JS works against the DOM.
...
- Long-term: faithful enough that standard JS frameworks (React, Vue) work unmodified.
...
packages/
  core/                          # @webium/core — shared TypeScript core
    src/
      ...
      parser/                    # HTML parser (HTML → VirtualDOM tree)
      api/                       # Browser API surface (document, element, event wrappers)
    tests/                       # vitest + fast-check property tests

src/
  Webium.Core/                   # Bridge interfaces + shared types (C#)
  Webium.JSRuntime/              # IJSRuntime/IJSBridge interfaces + engine-agnostic stubs (C#)
  Webium.Unity.Runtime/          # Unity rendering backend + UnityPuerTSRuntime (real PuerTS adapter)
...
| `ILayoutReadbackNew` | Provides engine-native layout values for synchronous JS queries |
...
The `Webium.JSRuntime` project defines `IJSRuntime` and `IJSBridge` interfaces. It contains only interfaces and engine-agnostic stubs — no real JS engine dependencies. `PuerTSRuntime` in this project is a stub for the `dotnet test` pipeline (all methods are no-ops).

The real PuerTS adapter lives in `Webium.Unity.Runtime` as `UnityPuerTSRuntime`, which implements `IJSRuntime` using `Puerts.JsEnv`. This keeps `Webium.JSRuntime` engine-agnostic (a future Godot backend could use Jint or another JS engine without touching this project).

Backend implementations:

- **PuerTS (Unity desktop/mobile):** `UnityPuerTSRuntime` in `Webium.Unity.Runtime` — V8 or QuickJS via PuerTS bindings. Requires the `com.tencent.puerts.core` UPM package.
- **Browser-native (WebGL):** `BrowserRuntime` — the browser's own JS engine via jslib interop.
- **Future:** Any JS engine with C# interop can implement `IJSRuntime`.
